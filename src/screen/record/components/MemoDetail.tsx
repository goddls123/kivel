...
import { GET_CHALLENGING_DETAIL, GET_DEVELOPMENT_DETAIL, GET_MEMO_DETAIL } from '../../../connection/queries';
import { challengingBehaviorType, developmentRecordType, memoType } from '../../../types/types';
...
export function MemoDetail(props : MemoDetailProps) {
	
	// Todo : emergency 추가
	const [detailInfo, setDetailInfo] = React.useState<memoType>()
	const {data , loading , error} = useQuery(GET_MEMO_DETAIL,{variables : {id : props.data.id}})
	React.useEffect(() => {
		if(data && !loading){
			setDetailInfo({...data.memo,...data.memoTags})
		}
	},[data])
	...
	// 	detailInfo?.image?.map((item : string, id : number) => {
	// 		viewArr.push( 
	// 			<ImageModal 
	// 			key={id} 
	// 			resizeMode='contain'
	...
	const renderDateText = () => {
		...
		return detailInfo?.occurenceDate.substr(0,10) + '(' + getDayKorean(date.getDay()) + ')'
	}
		...
		return (
			<View style={styles.container}>
			{
				...
				<View style={styles.topIconContainer}>
					...
					<Image style={styles.topImage} source={require('../../../assets/icons/ic_memo_40.png')}></Image>
					</View>
				</View>
				<View style={{paddingHorizontal : GLOBAL_MARGIN_HORIZON, marginBottom : GLOBAL_MARGIN_HORIZON}}>
					<Text style={styles.topText}>메모 <Text style={{color : 'black'}}>상세보기</Text></Text>
				</View>
				...
				<View style={styles.mainContainer}>
					...
					{
						detailInfo?.memoTags?.map((tag : any, id : number) => {
							return(
				...
				<View style={styles.buttonContainer}>
					...
					<View style={{flex : 1, marginLeft : 5}}>
						...
						onPress={() => {
							...
							props.navigation.navigate('AddRecord',{radioState : [false, false, true], memoData : detailInfo})
						}}
						textColor={'white'}
...
const styles = StyleSheet.create({
	...
	topText : {paddingTop : GLOBAL_MARGIN_HORIZON , fontSize : 20, fontWeight : 'bold', color : '#707070'},
