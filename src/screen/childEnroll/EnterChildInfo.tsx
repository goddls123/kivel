...
export function EnterChildInfo(props: enterChildInfoProps) {
    ...
    const setSex = (value : 'M' | 'W') => {
        setChildInfo({...childInfo, sex : value})
    }
    const setDiagnosis = (value : string) => {
        setChildInfo({...childInfo, diagnosis : value})
    }
    const setBirthWeekNum = (value : string) => {
		let intValue = parseInt(value)
        setChildInfo({...childInfo, birthWeekNum : intValue})
    }
    const setBirthDayNum = (value : string) => {
		let intValue = parseInt(value)
        setChildInfo({...childInfo, birthDayNum : intValue})
    }
    const setHeight = (value : string) => {
		let intValue = parseInt(value)
        setChildInfo({...childInfo, height : intValue})
    }
    const setWeight = (value : string) => {
		let intValue = parseInt(value)
        setChildInfo({...childInfo, weight : intValue})
    }
	const [diagnosisFreeText, setDiagnosisFreeText] = React.useState<string>()
    ...
    const essentialOptionCheck = (): boolean => {
        if (
            rightName.test(childInfo.name) &&
            childInfo.birthDate &&
            childInfo.sex &&
			childInfo.weight &&
            rightWeight(childInfo.weight) &&
			childInfo.birthWeekNum &&
            rightbirthWeekNum(childInfo.birthWeekNum) &&
			childInfo.birthDayNum &&
            rightbirthDayNum(childInfo.birthDayNum) &&
			childInfo.height &&
            rightHeight(childInfo.height) &&
			childInfo.diagnosis
        ){ return true }
            ...
            <View style={styles.innerContainer}>
                ...
                <ScrollView style={{flex: 1, paddingTop : SIZE_HEIGHT * 0.03}} showsVerticalScrollIndicator={false}>
                    ...
                    <TextInputView
                        ...
                        value={new Date(childInfo.birthDate)}></TextInputView>
                    ...
                    <View style={{marginBottom: SIZE_HEIGHT * 0.06}}>
                        ...
                        <View
                            ...
                            <TouchableOpacity
                                style={[
                                    styles.buttonSex,
                                    {borderColor: childInfo.sex == 'M' ? MAIN_COLOR : '#ededed'},
                                ]}
                                onPress={() => setSex('M')}>
                                <Text style={{color: childInfo.sex == 'M' ? MAIN_COLOR : '#aaaaaa'}}>
                                    남
                                </Text>
                            </TouchableOpacity>
                            ...
                            <TouchableOpacity
                                style={[
                                    styles.buttonSex,
                                    {borderColor: childInfo.sex == 'W' ? MAIN_COLOR : '#ededed'},
                                ]}
                                onPress={() => setSex('W')}>
                                <Text style={{color: childInfo.sex == 'W' ? MAIN_COLOR : '#aaaaaa'}}>
                                    여
                                </Text>
                            </TouchableOpacity>
                        </View>
                    ...
                    <TextInputView
                        ...
                        editable={false}
                        value={childInfo.diagnosis == "직접입력" ? "직접입력" : childInfo.diagnosis}></TextInputView>
                    
                    {
                        childInfo.diagnosis === '직접입력' ? 
                        <TextInputView
                            ...
                            onChangeText={setDiagnosisFreeText}></TextInputView>
                            : null
					}
                    ...
                    <Button
                        ...
                        onPress={() =>    
							props.navigation.navigate('NurseryCaution',
						 	{ 
								...childInfo, 
								diagnosis : childInfo.diagnosis == '직접입력' ? childInfo.diagnosis : diagnosisFreeText 
							})
						}
