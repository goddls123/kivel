...
export function holidayTextColor(
  day: Date,
  state: '' | 'disabled' | 'selected' | 'today',
) {
  if (state === 'disabled') return '#d9e1e8';
  else if (day.getDay() === 0) return 'red';
  else if (day.getDay() === 6) return 'blue';
  else return 'black';
}
...
export function scheduleCheck(date: Date, schedule: schedule) {
  if (schedule.repeat_cycle === 'D') {
    ...
    if (schedule.schedule_date[date.getDay()] === '1') return true;
  } else if (schedule.repeat_cycle === 'M') {
    if (date.getDate().toString() === schedule.schedule_date) return true;
