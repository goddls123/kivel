...
import {
  ...
  getDayKorean,
  getTime,
} from '../common/service/dateService';
...
export function addCalendarPage(props: addCalendarPageProps) {
  ...
  const [daySelected, setDaySelected] = React.useState<boolean[]>([
    false,
    false,
    false,
    false,
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        <Animated.View>
          ...
          {/* 시작 */}
          <TouchableOpacity
            style={styles.pickerViewOpenButton}
            onPress={() => {
        ...
        <View style={{marginTop: 10, justifyContent: 'center'}}>
          ...
          <Animated.View
            style={[styles.daySelector, {maxHeight: daySelectorHeight}]}>
            {daySelected.map((item: boolean, id: number) => {
              return (
                <View
                  ...
                  <TouchableOpacity
                    style={[
                      styles.daySelectorBubble,
                      {backgroundColor: item ? MAIN_COLOR : '#ededed'},
                    ]}
                    onPress={() => selectDay(id)}>
                    <Text
                      style={[
                        styles.weekText,
                        {color: item ? 'white' : '#aaaaaa'},
                      ]}>
                      {getDayKorean(id)}
                    </Text>
                  </TouchableOpacity>
        ...
        <View
          ...
          <Button
            text={'완료'}
            textColor={'white'}
            style={{backgroundColor: MAIN_COLOR}}></Button>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  daySelector: {
    flexDirection: 'row',
    marginHorizontal: GLOBAL_MARGIN_HORIZON * 2,
    justifyContent: 'center',
    overflow: 'hidden',
  },
  daySelectorBubble: {
    height: SIZE_WIDTH * 0.08,
    width: SIZE_WIDTH * 0.08,
    borderRadius: 100,
    alignItems: 'center',
    justifyContent: 'center',
  },
